...
codes = huffman_codes(bwt_x)
wt, pointers = lop.wavelet_tree(bwt_x, codes)
ranks = lop.all_node_ranks(wt, len(bwt_x), pointers)

def test_AACGTAAACGTAAC_AAC():
	p = "AAC"
	ls = [0, 6, 11]
	assert bwt_O.bw_search(bwt_x, p, sparse_sa, C, O, letter_to_num_dict) == ls
	assert bwt_wt.bw_search(bwt_x, p, n, sparse_sa, C_dict, wt, pointers, ranks, codes) == ls
